...
INDEX_FILE = Path(__file__).resolve().parent / "magic_index.json"
...
def load_index() -> dict:
    ...
    try:
        ...
        logger.error("magic_index.json corrupted, resetting to empty index")
        return {"guilds": {}}
    except OSError:
        logger.error("Failed to read magic_index.json", exc_info=True)
        return {"guilds": {}}
...
def save_index(data: dict) -> None:
    ...
    try:
        ...
        logger.error("Failed to write magic_index.json", exc_info=True)
